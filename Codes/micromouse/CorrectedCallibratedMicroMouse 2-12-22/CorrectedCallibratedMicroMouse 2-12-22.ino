...
void realign() {
  if (i3 > 200 && i4 > 200) {
    ...
    while (e > 0) {
      if (i3 < i4) {
        setMotor(1, 100, M1A, M1B);
        setMotor(-1, 100, M2A, M2B);
      } else {
  ...
  } else if (i1 < 100) {  //right wall is absent
  }
}
...
void drive() {
  ...
  int e1 = posA - target1;
  int e2 = posB - target2;
  if (p == 1 && fabs(e1) < 20 && fabs(e2) < 20) { //forward stop
    noInterrupts();
    analogWrite(M1A, 0);
    analogWrite(M1B, 0);
    analogWrite(M2A, 0);
    ...
    interrupts();
    delay(1000);
  } else if (fabs(e1) < 15 && fabs(e2) < 15) {  //stop otherwise
    noInterrupts();
    analogWrite(M1A, 0);
    analogWrite(M1B, 0);
    analogWrite(M2A, 0);
    ...
    if (p == 1) { // driving forward
      Serial.print("here");
      if (i3 > 100 && i4 > 100) {  // forward wall
        analogWrite(M1A, 0);
        analogWrite(M1B, 0);
        analogWrite(M2A, 0);
        analogWrite(M2B, 0);
        int e1 = i3 - 600;
        int e2 = i4 - 900;
        while (fabs(e1) > 20 || fabs(e2) > 20) {  /// realign with front wall
          readIR();
          dir1 = 1;
          dir2 = 1;
          if (e1 < 0) {
          ...
          if (fabs(e1) > 20) {
            setMotor(dir1, 50, M2A, M2B);
          }
          if (fabs(e2) > 20) {
            setMotor(dir2, 50, M1A, M1B);
          }
          e1 = i3 - 600;
          e2 = i4 - 900;
        }
        setMotor(dir2, 0, M1A, M1B);
        setMotor(dir1, 0, M2A, M2B);
        delay(200);
        noInterrupts();
        pos1 = 0;
        pos2 = 0;
        ...
        interrupts();
        delay(2000);
      } else if (i2 > 100) {  //left wall present
        int e = fabs(i2 - ia2);
        dei1 = e;
        if (e > 50) {
          if (i2 > ia2) {
            pwr2 += min(100, 1.2 * e);  // fabs(i1 - ia1) * 0;
            pwr1 -= 0.1 * e;
          } else {
            pwr1 += min(100, 1.2 * e);  // fabs(i1 - ia1) * 0;
            pwr2 -= 0.2 * e;
          }
        }
        if (pwr1 > 255) {
     ...
     else if (i1 > 100) {  // right wall is present
      int e = fabs(i1 - ia1);
      dei2 = e;
      if (e > 50) {
        if (i1 < ia1) {
          pwr2 += min(100, 1.2 * e);  //  e * 0 + (dei2 - e) * 0.0;
          pwr1 -= 0.1 * e;
        } else {
          pwr1 += min(100, 1.2 * e);  //  e * 0 + (dei2 - e) * 0.0;
          pwr2 -= 0.2 * e;
        }
        te += e;
      }
